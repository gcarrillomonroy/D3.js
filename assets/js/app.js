...
function xScale(fileData, chosenXAxis) {
  ...
  var xLinearScale = d3.scaleLinear()
    .domain([d3.min(fileData, d => d[chosenXAxis]) * 0.8,
      d3.max(fileData, d => d[chosenXAxis]) * 1.2
    ])
    .range([0, width]);
...
d3.csv("./assets/data/data.csv", function(err, fileData) {
  if (err) throw err;
  ...
  labelsGroup.selectAll("text")
    .on("click", function() {
      ...
      if (value !== chosenXAxis) {
        ...
        //console.log("EJE:" + chosenXAxis);

        // changes classes to change bold text

        //console.log("AXIS=",chosenXAxis);
